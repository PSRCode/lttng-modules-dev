...
EXPORT_SYMBOL_GPL(lttng_pid_filter_lookup);

/*
 * Filter add and del operations support concurrent RCU lookups.
 */
...
static
void pid_filter_del_node_rcu(struct lttng_pid_hash_node *e)
{
	hlist_del_rcu(&e->hlist);
	/*
	 * We choose to use a heavyweight synchronize on removal here,
	 * since removal of a PID from the filter mask is a rare
 ...
 */
static
void pid_filter_del_node(struct lttng_pid_hash_node *e)
{
	hlist_del(&e->hlist);
	kfree(e);
}
...
{
	return kzalloc(sizeof(struct lttng_pid_filter), GFP_KERNEL);
}
